...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'AutoRest.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'AutoRest.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'AutoRest.Client.ProtectedToken'
