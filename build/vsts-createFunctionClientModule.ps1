...
$parentModule = 'AutoRest'
if (-not $ModuleName) { $ModuleName = 'AutoRest.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\AutoRest'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\AutoRest.psd1"
$prereqHash = @{
